...
import { PropertyOwner } from '../FinancialPositionForm';

interface PropertyOwnersProps {
  owners: PropertyOwner[];
  readOnly?: boolean;
  onAddOwner: () => void;
  onUpdateOwner: (id: string, field: keyof PropertyOwner, value: string) => void;
...
const PropertyOwners: React.FC<PropertyOwnersProps> = ({
  ...
  const prevCountRef = useRef(owners.length);

  useEffect(() => {
    if (owners.length > prevCountRef.current && lastItemRef.current) {
      setTimeout(() => {
        lastItemRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
      }, 650);
    }
    prevCountRef.current = owners.length;
  }, [owners.length]);
      ...
      {owners.length === 0 ? (
        ...
        <div className="space-y-2.5">
          {owners.map((owner, index) => (
            <div
              key={owner.id}
              ref={index === owners.length - 1 ? lastItemRef : null}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="email"
                    value={owner.email}
                    onChange={(e) => onUpdateOwner(owner.id, 'email', e.target.value)}
                    disabled={readOnly}
                    className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-fundtap-primary/20 focus:border-fundtap-primary focus:outline-none transition-colors duration-200 bg-white"
                ...
                <div>
                  ...
                  <input
                    type="tel"
                    value={owner.phone}
                    onChange={(e) => onUpdateOwner(owner.id, 'phone', e.target.value)}
                    disabled={readOnly}
                    className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-fundtap-primary/20 focus:border-fundtap-primary focus:outline-none transition-colors duration-200 bg-white"
